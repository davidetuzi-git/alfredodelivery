...
serve(async (req) => {
  ...
  try {
    ...
    const { amount, orderData, returnUrl, cancelUrl } = await req.json();
    logStep("Request body parsed", { amount, hasOrderData: !!orderData });
    ...
    // Create checkout session with price_data for dynamic amount
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: "Ordine Alfredo",
              description: `Ordine spesa - ${orderData?.itemCount || 0} articoli`,
            },
